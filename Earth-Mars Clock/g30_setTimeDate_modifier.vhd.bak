...
entity g30_setTimeDate_modifier is
...
end g30_setTimeDate_modifier;

architecture behavior of g30_setTimeDate_modifier is
...
process (EHours, zoneNum, DSTsw9,Days,Months,Years,ESeconds, EMinutes, Daylim_present, Daylim_past)
begin
...
--	if (Unsigned(EHours) - zoneNum -DST )> "10111" then
--		earthHour<=(Unsigned(EHours) - zoneNum +DST - "11000");
...
--		end if;
	...
	if (Unsigned(EHours))< (zoneNum + DST) then
		earthHours<=(Unsigned(EHours) +("11000" -DST - zoneNum));
		if Days="00001" then
	...
	if (Unsigned(EHours) + ("11000" - zoneNum -DST) )> "11000" then
		earthHours<= (Unsigned(EHours) - zoneNum -DST);	
		if Days=(Daylim_present) then 
			EDays<="00001";
			if Months="1100" then 
		...
		earthHours<= (Unsigned(EHours) + ("11000" - zoneNum -DST) );
		EDays<=unsigned(Days);
		EMonths<=unsigned(Months);
		EYears<=unsigned(Years);
